...
export default function InferenceTab() {
    ...
    const formattedLastRun = lastRunAt ? formatDateTime(lastRunAt, i18n.language) : null;
    ...
    return (
        <div className="h-full flex flex-col gap-4 p-4 bg-background">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <div className="text-xs uppercase tracking-wide text-muted-foreground">{t('canvas.inference.workspaceLabel')}</div>
                    <h2 className="text-xl font-semibold text-foreground flex items-center gap-2"><Cpu size={18} /> {t('canvas.inference.title')}</h2>
                </div>
                <div className="text-sm text-muted-foreground">
                    {canPredict ? t('canvas.inference.readyToPredict') : t('canvas.inference.trainToUnlock')}
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
                ...
                <div className="bg-card border border-border rounded-lg p-4 space-y-2">
                    <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{t('canvas.inference.datasetLabel')}</div>
                    <div className="text-sm font-medium text-foreground">{dataset?.datasetName || t('canvas.inference.activeDataset')}</div>
                    {dataset?.totalRows && dataset?.totalColumns && (
                        <div className="text-xs text-muted-foreground">{dataset.totalRows} {t('canvas.inference.rows')} • {dataset.totalColumns} {t('canvas.inference.features')} • {t('canvas.inference.target')}: {dataset?.preprocessingConfig?.targetColumn || t('canvas.loadModal.na')}</div>
                    )}
                </div>
                <div className="bg-card border border-border rounded-lg p-4 space-y-2">
                    <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{t('canvas.inference.normalizationLabel')}</div>
                    <div className="text-sm font-medium text-foreground">{normalizationMethod === 'none' ? t('canvas.inference.notApplied') : normalizationMethod}</div>
                    <div className="text-xs text-muted-foreground">{normalizationMethod === 'none' ? t('canvas.inference.sendRaw') : t('canvas.inference.matchScaling')}</div>
                </div>
            </div>
                ...
                {!canPredict ? (
                    <div className="h-full flex items-center justify-center">
                        <div className="bg-card border border-border rounded-xl p-10 text-center max-w-md">
                            ...
                            <h3 className="text-lg font-semibold text-foreground mb-2">{t('canvas.inference.noModel')}</h3>
                            <p className="text-sm text-muted-foreground">{t('canvas.inference.finishTraining')}</p>
                        </div>
                    </div>
                ) : (
                    <div className="h-full grid gap-4 lg:grid-cols-2">
                        <div className="bg-card border border-border rounded-lg flex flex-col overflow-hidden">
                            <div className="border-b border-border px-4 py-3">
                                <div className="text-xs uppercase tracking-wide text-muted-foreground">{t('canvas.inference.step')} 1</div>
                                <div className="flex items-center gap-2 text-base font-semibold text-foreground">{t('canvas.inference.prepareInput')}</div>
                            </div>
                            <div className="flex-1 overflow-auto px-4 py-4 space-y-4">
                                {featureList.length > 0 && (
                                    <div className="text-xs text-muted-foreground">
                                        <div className="mb-1 font-medium">{t('canvas.inference.expectedFeatureOrder', { count: featureList.length })}</div>
                                ...
                                <div className="space-y-2">
                                    <div className="text-xs font-medium text-muted-foreground uppercase">{t('canvas.inference.inputFormat')}</div>
                                ...
                                {mode === 'json' ? (
                                    <div className="space-y-2">
                                        <div className="text-xs text-muted-foreground">
                                            {t('canvas.inference.pasteJsonInstruction')} <code>{'{"instances": [...]}'}</code>.
                                        </div>
                                        {hasNormalization && (
                                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                                                <input type="checkbox" checked={applyNormalization} onChange={(e) => setApplyNormalization(e.target.checked)} />
                                                {t('canvas.inference.applyNormalization', { method: normalizationMethod })}
                                            </label>
                                        )}
                                        <textarea
                                            value={jsonText}
                                        ...
                                        <div className="flex flex-wrap gap-2 text-xs">
                                            ...
                                            >
                                                <Download size={12} /> {t('canvas.inference.jsonTemplate')}
                                            </button>
                                            <div className="inline-flex items-center gap-1 text-muted-foreground"><Info size={12} /> {t('canvas.inference.keepOrderNote')}</div>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="space-y-3">
                                            ...
                                            {csvFile ? (
                                                <div className="space-y-2">
                                                    ...
                                                    <div className="flex gap-2 text-xs">
                                                        <button type="button" onClick={() => csvInputRef.current?.click()} className="rounded border border-border px-2 py-1 hover:bg-accent">{t('canvas.inference.replaceFile')}</button>
                                                        <button type="button" onClick={() => setCsvFile(null)} className="rounded border border-border px-2 py-1 hover:bg-accent">{t('canvas.inference.removeFile')}</button>
                                                    </div>
                                                </div>
                                            ) : (
                                                <div className="space-y-2">
                                                    <div>{t('canvas.inference.selectCsvHint')}</div>
                                                    <button type="button" onClick={() => csvInputRef.current?.click()} className="rounded bg-card px-3 py-1 text-sm font-medium text-primary shadow-sm border border-border hover:bg-accent">{t('canvas.inference.chooseFile')}</button>
                                                </div>
                                            )}
                                        ...
                                        >
                                            <Download size={12} /> {t('canvas.inference.downloadCsvTemplate')}
                                        </button>
                                    </div>
                                )}
                                ...
                                <div className="rounded-lg bg-muted/30 p-3 text-xs text-muted-foreground flex gap-2">
                                    ...
                                    <div>
                                        {t('canvas.inference.keepTargetNote')}
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-col gap-2">
                                    ...
                                    >
                                        <Play size={16} /> {busy ? t('canvas.inference.predicting') : t('canvas.inference.runInference')}
                                    </button>
                        ...
                        <div className="bg-card border border-border rounded-lg flex flex-col overflow-hidden">
                            <div className="border-b border-border px-4 py-3 flex flex-wrap items-center justify-between gap-2">
                                <div>
                                    <div className="text-xs uppercase tracking-wide text-muted-foreground">{t('canvas.inference.step')} 2</div>
                                    <div className="text-base font-semibold text-foreground">{t('canvas.inference.reviewPredictions')}</div>
                                    {formattedLastRun && <div className="text-xs text-muted-foreground">{t('canvas.inference.lastRun')}: {formattedLastRun}</div>}
                                </div>
                                <div className="flex gap-2">
                                    ...
                                    >
                                        <ClipboardCopy size={12} /> {copied ? t('canvas.inference.copied') : t('canvas.inference.copyJson')}
                                    </button>
                                    <button
                                        type="button"
                                        ...
                                        className="inline-flex items-center gap-1 rounded border border-border px-2 py-1 text-xs text-muted-foreground disabled:opacity-50 hover:bg-accent"
                                    >
                                        <Download size={12} /> {t('canvas.inference.download')}
                                    </button>
                                </div>
                            </div>
                            <div className="flex-1 overflow-auto p-4 space-y-3">
                                {!preds ? (
                                    <div className="h-full flex items-center justify-center text-sm text-muted-foreground">
                                        {t('canvas.inference.runPredictionHint')}
                                    </div>
                                ) : (
                                        ...
                                        {predictionSummary && (
                                            <div className="flex flex-wrap gap-2 text-xs">
                                                <span className="rounded-full bg-emerald-50 dark:bg-emerald-950 px-3 py-1 text-emerald-700 dark:text-emerald-200">
                                                    {predictionSummary.total} {t('canvas.inference.records')}
                                                </span>
                                                {typeof predictionSummary.vectorLength === 'number' && (
                                                    <span className="rounded-full bg-blue-50 dark:bg-blue-950 px-3 py-1 text-blue-700 dark:text-blue-200">
                                                        {predictionSummary.vectorLength} {t('canvas.inference.outputsPerRecord')}
                                                    </span>
                                                )}
                                            </div>
                                        )}
                                        ...
                                        {(() => {
                                            ...
                                            return (
                                                <div className="text-xs text-foreground space-y-2">
                                                    <div className="font-medium">{t('canvas.inference.argmaxIndices')}</div>
                                                    <pre className="bg-muted/30 border border-border rounded p-2">{JSON.stringify(classes)}</pre>
                                                    {labels ? (
                                                        <div className="text-xs">
                                                            <div className="font-medium mb-1">{t('canvas.inference.mappedLabels')}</div>
                                                            <div className="flex flex-col gap-1">
                                                                {classes.map((idx: number, i: number) => (
                                                                    <div key={i} className="flex items-center gap-2">
                                                                        <div className="w-6 text-muted-foreground">#{idx}</div>
                                                                        <div className="font-medium">{labels[idx] ?? t('canvas.inference.unknown')}</div>
                                                                    </div>
                                                                ))}
                                                            </div>
                                                            <div className="mt-2 text-muted-foreground">{t('canvas.inference.fullMapping')}</div>
                                                        ...
                                                        <div className="text-muted-foreground">{t('canvas.inference.mapIndicesHint')}</div>
                                                    )}
                                                </div>
                                            );
                                        })()}
